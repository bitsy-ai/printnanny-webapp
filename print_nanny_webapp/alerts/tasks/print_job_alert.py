...
def create_video(annotated_images, alert_id):
    ...
    file.name = filename
    imageio.imwrite(file, [
        Image(img_bytes) for img_bytes in annotated_images
    ])
...
def create_progress_video_task(print_job_id, user_id, progress, fps=10):
    ...
    video_task = group([
        download_frame.si(e) for e in predict_events
    ]) | create_video.s(alert.id)
    ...
    workflow = group(
        video_task,
        create_report_card.si(df, alert.id, temp_dir, fps, callback)
    )
