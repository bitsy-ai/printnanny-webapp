...
def create_remote_control_command_alerts(user_id, command_id, alert_subtype):
    ...
    if command is None:
        raise InvalidCommand(f'Command id={command_id} NOT FOUNDs while processing {alert_subtype} alerts for user_id={user_id}')

    logging.info(f'Processing {alert_subtype} alerts for command={command.command} command_id={command_id} user_id={user_id}')
