...
<template>
    <Popover class="relative" v-slot="{ open }">
      <PopoverButton :class="[open ? 'text-white' : 'text-white', 'group inline-flex items-center rounded-md bg-gray-900 text-base font-medium hover:text-indigo-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2']">
        <span>{{ menuText }}</span>
        ...
        <PopoverPanel class="absolute left-1/2 z-10 mt-3 w-screen max-w-xs -translate-x-1/2 transform px-2 sm:px-0">
          <div class="overflow-hidden rounded-lg shadow-lg ring-1 ring-black ring-opacity-5">
            <div class="relative grid gap-6 bg-gray-900 px-5 py-6 sm:gap-8 sm:p-8">
              <span v-for="item in links" :key="item.name" class="-m-3 block rounded-md p-3 transition duration-150 ease-in-out hover:bg-gray-800">
                <a v-if="item.href && !item.blank" :href="item.href">
                  <p class="text-base font-medium text-white">{{ item.name }}</p>
                  <p class="mt-1 text-sm text-gray-500">{{ item.description }}</p>
                </a>
                <a v-else-if="item.href && item.blank" :href="item.href" target="_blank">
                  <p class="text-base font-medium text-white">{{ item.name }}</p>
                  <p class="mt-1 text-sm text-gray-500">{{ item.description }}</p>
                </a>
                <router-link v-else-if="item.routerLocation" :to="item.routerLocation">
                  <p class="text-base font-medium text-white">{{ item.name }}</p>
                  <p class="mt-1 text-sm text-gray-500">{{ item.description }}</p>
                </router-link>
              </span>
