    ...
    <div class="bg-gray-50">
      <div class="relative shadow bg-gradient-to-r from-indigo-500 to-violet-600">
        ...
        <div class="relative mx-auto max-w-2xl px-6 pt-16 text-center sm:pt-32 lg:max-w-7xl lg:px-8">
          ...
          <p class="mt-4 text-xl text-indigo-100">Pay-as-you-go monthly plans.</p>
          <p class="mt-4 text-xl text-indigo-100">Unlimited annual plans that scale with your business.</p>
            ...
            <div v-for="plan in plans" :key="plan.title" :class="[plan.featured ? 'bg-white ring-2 ring-indigo-700 shadow-md' : 'bg-indigo-700 lg:bg-transparent', 'pt-6 px-6 pb-3 rounded-lg lg:px-8 lg:pt-12']">
              <div>
                ...
                <div class="flex flex-col items-start sm:flex-row sm:items-center sm:justify-between lg:flex-col lg:items-start">
                  <div class="mt-3 flex items-center" v-if="plan.priceOnRequest">
                    <p :class="[plan.featured ? 'text-gray-500' : 'text-indigo-200', 'text-sm']">{{  plan.priceOnRequest }}</p>
                  </div>
                  ...
                  <a :href="plan.href" :class="[plan.featured ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'bg-white text-indigo-600 hover:bg-indigo-50', 'mt-6 w-full inline-block py-2 px-8 border border-transparent rounded-md shadow-sm text-center text-sm font-medium sm:mt-0 sm:w-auto lg:mt-6 lg:w-full']">{{  plan.cta  }}</a>
                </div>
              </div>
      ...
      <section aria-labelledby="mobile-comparison-heading" class="lg:hidden">
        ...
        <div class="mx-auto max-w-2xl space-y-16 py-16 px-6">
          <div v-for="(plan, mobilePlanIndex) in plans" :key="mobilePlanIndex" class="border-t border-gray-200">
            ...
            <h4 class="mt-10 text-sm font-bold text-gray-900">Automation for everyone</h4>
        ...
        <div class="mx-auto max-w-7xl py-24 px-8">
          <div class="flex w-full items-stretch border-t border-gray-200">
            <div class="-mt-px flex w-1/4 items-end py-6 pr-4">
              <h3 class="mt-auto text-sm font-bold text-gray-900">Automation for everyone</h3>
            </div>
            ...
            <table class="relative w-full">
              ...
              <tbody class="divide-y divide-gray-100">
                <tr v-for="feature in features" :key="feature.title">
                  <th scope="row" class="w-1/5 py-3 pr-4 text-left text-sm font-medium text-gray-600">{{ feature.title }}</th>
                  <td v-for="(tier, tierIdx) in feature.tiers" :key="tier.title" :class="[tierIdx === feature.tiers.length - 1 ? 'pl-4' : 'px-4', 'relative w-1/5 py-0 text-center']">
            ...
            <table class="relative w-full">
              ...
              <tbody class="divide-y divide-gray-100">
                <tr v-for="perk in perks" :key="perk.title">
                  <th scope="row" class="w-1/5 py-3 pr-4 text-left text-sm font-medium text-gray-600">{{ perk.title }}</th>
                  <td v-for="(tier, tierIdx) in perk.tiers" :key="tier.title" :class="[tierIdx === perk.tiers.length - 1 ? 'pl-4' : 'px-4', 'relative w-1/5 py-0 text-center']">
                    <span class="relative h-full w-full py-3">
                      <CheckIcon v-if="tier.value === true" class="mx-auto h-5 w-5 text-indigo-600" aria-hidden="true" />
                      <XMarkIcon v-else class="mx-auto h-5 w-5 text-gray-400" aria-hidden="true" />
                      <span class="sr-only">{{ tier.value === true ? 'Yes' : 'No' }}</span>
  ...
  const plans = [
    ...
    //   priceYearly: 140,
      mainFeatures: [
        ...
        { id: 2, value: 'Customized cloud + operating system' },
    ...
    },
    {
      ...
      cta: 'Get a Quote',
      featured: false,
      description: 'Customized for schools, libraries, research labs, and makerspaces.',
      priceOnRequest: 'Customized for schools, libraries, research labs, and makerspaces.',
    //   priceMonthly: 12,
    //   priceYearly: 140,
      mainFeatures: [
