...
export const useDemoStore = defineStore({
    ...
    actions: {
        ...
        async submit(email: string, filename: string) {
            const account = useAccountStore();
            const res = await account.demosApi.demosCreate(email, filename).catch(handleApiError);
            if (res) {
                success("Success! Your submission is queued", `We'll email you at ${email} when your results are ready.`)
            }
        },
