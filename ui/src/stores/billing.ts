...
export const useBillingStore = defineStore({
    ...
    actions: {
        async cancel() {
            ...
            try {
                const res = await billingApi.billingCancelCreate(this.summary.subscription.id);
                console.log("Cancelled subscription, updated billing summary: ", res.data);
                const alert: UiAlert = {
                    header: "Subscription was cancelled",
                    message: "Sorry to see you go! Email support@printnanny.ai if you need any further assistance."
                };
                alerts.push(alert);
        ...
        async reactivate() {
            const alerts = useAlertStore();
            try {
                const res = await billingApi.billingReactivateCreate(this.summary.subscription.id);
                console.log("Reactivated subscription, updated billing summary: ", res.data);
                const alert: UiAlert = {
                    header: "Subscription reactivated",
                    message: "Welcome back! Email support@printnanny.ai if you need any further assistance."
                };
                alerts.push(alert);
                this.$patch({ summary: res.data });
                this.router.push({ name: "billing" });
