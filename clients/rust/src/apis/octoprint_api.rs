...
/// struct for typed errors of method [`devices_octoprint_list`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum DevicesOctoprintListError {
...
pub async fn devices_octoprint_list(configuration: &configuration::Configuration, device_id: i32, page: Option<i32>) -> Result<crate::models::PaginatedOctoPrintServerList, Error<DevicesOctoprintListError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/api/devices/{device_id}/octoprint/", local_var_configuration.base_path, device_id=device_id);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<DevicesOctoprintListError> = serde_json::from_str(&local_var_content).ok();
