...
/// struct for typed errors of method [`pis_events_list2`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum PisEventsList2Error {
    Status400(crate::models::ErrorDetail),
    Status401(crate::models::ErrorDetail),
    Status403(crate::models::ErrorDetail),
    Status500(crate::models::ErrorDetail),
...
/// Interact with all events inheriting from BasePiEvent
pub async fn pis_events_list(configuration: &configuration::Configuration, page: Option<i32>) -> Result<crate::models::PaginatedPolymorphicPiEventList, Error<PisEventsListError>> {
    let local_var_configuration = configuration;
...
/// Interact with all events inheriting from BasePiEvent
pub async fn pis_events_list2(configuration: &configuration::Configuration, pi_id: i32, page: Option<i32>) -> Result<crate::models::PaginatedPolymorphicPiEventList, Error<PisEventsList2Error>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<PisEventsList2Error> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
