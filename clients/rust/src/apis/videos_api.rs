...
/// struct for typed errors of method [`video_recordings_get_or_create`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum VideoRecordingsGetOrCreateError {
    Status409(crate::models::ErrorDetail),
    Status400(crate::models::ErrorDetail),
    Status401(crate::models::ErrorDetail),
    Status403(crate::models::ErrorDetail),
...
pub async fn video_recordings_get_or_create(configuration: &configuration::Configuration, id: Option<&str>, recording_start: Option<String>, recording_end: Option<String>, recording_status: Option<crate::models::RecordingStatusEnum>, cloud_sync_start: Option<String>, cloud_sync_end: Option<String>, cloud_sync_status: Option<crate::models::CloudSyncStatusEnum>, gcode_file_name: Option<&str>, mp4_file: Option<std::path::PathBuf>) -> Result<crate::models::VideoRecording, Error<VideoRecordingsGetOrCreateError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/api/video-recordings/get-or-create/", local_var_configuration.base_path);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::POST, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<VideoRecordingsGetOrCreateError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
