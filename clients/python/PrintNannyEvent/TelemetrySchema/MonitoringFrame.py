...
class MonitoringFrame(object):
    ...
    def BoundingBoxes(self):
        ...
        if o != 0:
            x = self._tab.Indirect(o + self._tab.Pos)
            from PrintNannyEvent.TelemetrySchema.BoundingBoxes import BoundingBoxes
            obj = BoundingBoxes()
            obj.Init(self._tab.Bytes, x)
            return obj
        return None
...
import PrintNannyEvent.TelemetrySchema.BoundingBoxes
...
class MonitoringFrameT(object):
    ...
    def __init__(self):
        ...
        self.boundingBoxes = None  # type: Optional[PrintNannyEvent.TelemetrySchema.BoundingBoxes.BoundingBoxesT]
    ...
    def _UnPack(self, monitoringFrame):
        ...
        if monitoringFrame.Image() is not None:
            self.image = PrintNannyEvent.TelemetrySchema.Image.ImageT.InitFromObj(monitoringFrame.Image())
        if monitoringFrame.BoundingBoxes() is not None:
            self.boundingBoxes = PrintNannyEvent.TelemetrySchema.BoundingBoxes.BoundingBoxesT.InitFromObj(monitoringFrame.BoundingBoxes())
